...
const envPath = path.join(__dirname, 'config.mainnet.env');
...
const app = express();
const PORT = envVars.PORT || 8600;
const ENVIRONMENT = envVars.ENVIRONMENT || 'mainnet';
...
app.get('/transaction/:txnHash', async (req, res) => {
  try {
    ...
    const query = 'SELECT * FROM tx_history_v2 WHERE transaction_hash = $1';
...
app.get('/transactions', async (req, res) => {
  try {
    ...
    // Get transactions
    const query = 'SELECT * FROM tx_history_v2 ORDER BY transaction_hash LIMIT $1 OFFSET $2';
    const result = await pool.query(query, [limit, offset]);

    // Get total count
    const countQuery = 'SELECT COUNT(*) FROM tx_history_v2';
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`🚀 Mainnet Server running on port ${PORT}`);
